...
def displayGaussians(next):
    ...
    print(no_agents)
    g = np.loadtxt("./heterogeneous_EC/saved data/gaussians/g.txt")
    gaussians = [[[]]]
    j = 0
    k = 0
    for element in g:
...
def displayPathsonInfoandProbability(next, live):
    (a,i,j,k,l,o) = next
    im = np.loadtxt("./heterogeneous_EC/saved data/info maps/infoMapDistributionData.txt")
    infoMaps = [[[]]]
    j = 0
    k = 0
    for element in im:
    ...
    sf = np.loadtxt("./heterogeneous_EC/saved data/agents/sensor_footprint.txt")
    sensorFootprints = [[],[],[],[],[],[],[],[],[],[]]
    ...
    for x in sensorFootprints:
        if len(x) == 0:
            x.append(1.0)
    s = np.loadtxt("./heterogeneous_EC/saved data/paths/data2.txt")
    points = [[]]
    j = 0
...
def displayPathsonProbability(next, live):
    ...
    plt.pause(10)
    sf = np.loadtxt("./heterogeneous_EC/saved data/agents/sensor_footprint.txt")
    print("Length of sensor footprint: ", len(sf))
    sensorFootprints = [[],[],[],[],[],[],[],[],[],[]]
    for i in range(len(sf)):
        if (sf[i] != -1):
            sensorFootprints[i].append(sf[i])
