...
class PrimitivePathPlanner():
    ...
    def registerNewAgent(self, agent):
        ...
        '''
        print("Registering a new agent: ", agent.splitMapIndex)
        print("The agent is shown as: ", agent)
    ...
    def replanPaths(self, map, time):
        ...
        #erg = mathlib.L1_error(map, infoMap)
        #print(erg)
    ...
    def getErgodicity(self, map):
        infoMap = self.generateCurrentInfoMap(map, 1)
        return(mathlib.calcErgodicity(map, infoMap))
    ...
    def generatePrimitivePaths(self, map, splitMapAgents, tries):
        ...
        for splitMapIndex in range(len(splitMapAgents)):
            ...
            for trial in range(tries * len(agentList)):
                ...
                for i in range(len(agentList)):
                    ...
                    reconstructedmap = Map(len(reconstructedmap), len(reconstructedmap[0]), map.dX, map.dY, 50, reconstructedmap)
                    count = 0
                    while inBounds == False:
                        ...
                        if (count > 7):
                            ...
                            (path, inBounds) = generator.generateRandomPrimitivePath(map, currentAgent) #reconstructed
                    paths.append(path)
                ...
                if minErg < 0 or abs(erg) < abs(minErg):
                    minErg = erg
        ...
        for splitMapIndex in range(len(splitMapAgents)):
            ...
            agentList = splitMapAgents[splitMapIndex]
            for trial in range(tries * len(agentList)):
                map = Map(originalMap.sizeX, originalMap.sizeY, originalMap.dX, originalMap.dY, 50,
                ...
                for n in range(10):
                    ...
                    if (len(map._distribution) <= highesty) or (len(map._distribution[0]) <= highestx):
                        ...
                        if (len(originalMap._distribution[int(lowesty):int(highesty),
                                int(lowestx):int(highestx)]) == 0):
                            map = Map(map.sizeX, map.sizeY, map.dX, map.dY, 50, map._distribution)
                        elif (len(originalMap._distribution[int(lowesty):int(highesty), int(lowestx):int(highestx)][
                                      0]) == 0):
                            map = Map(map.sizeX, map.sizeY, map.dX, map.dY, 50, map._distribution)

                        # Map stays unchanged if sum of probabilities in new map is 0.0
                        elif (originalMap._distribution[int(lowesty):int(highesty),
                              int(lowestx):int(highestx)].sum() == 0.0):
                            map = Map(map.sizeX, map.sizeY, map.dX, map.dY, 50, map._distribution)

                        # Map stays unchanged if new map dimensions is less than 5x5
                        elif ((len(originalMap._distribution[int(lowesty):int(highesty),
                                   int(lowestx):int(highestx)]) * len(
                                originalMap._distribution[int(lowesty):int(highesty), int(lowestx):int(highestx)][
                                    0])) < 25):
                            map = Map(map.sizeX, map.sizeY, map.dX, map.dY, 50, map._distribution)
                            ...
                            for i in range(len(
                                    originalMap._distribution[int(lowesty):int(highesty), int(lowestx):int(highestx)])):
                                if (originalMap._distribution[int(lowesty):int(highesty), int(lowestx):int(highestx)][
                                    i].sum() == 1.0):
                                    map = Map(originalMap.sizeX, originalMap.sizeY, originalMap.dX, originalMap.dY, 50,
                                              originalMap._distribution)
                        # Map stays unchanged if only one value in new map is non-zero
                            ...
                            if flag >= (len(originalMap._distribution[int(lowesty):int(highesty),
                                            int(lowestx):int(highestx)]) - 3):
                                map = Map(originalMap.sizeX, originalMap.sizeY, originalMap.dX, originalMap.dY, 50,
                                          originalMap._distribution)
                            else:
                                map = Map(abs(int(highestx) - int(lowestx)), abs(int(highesty) - int(lowesty)), map.dX,
                                          map.dY, 50, originalMap._distribution[int(lowesty):int(highesty),
