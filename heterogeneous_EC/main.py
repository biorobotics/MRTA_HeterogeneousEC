...
def StartSimulation():
    testMap = Map(100, 100, 1, 1, 50, np.loadtxt("./heterogeneous_EC/saved data maps/notrand" + str(5) + ".txt")[:100])
...
def exportObservations(simulationData, sim):
    ...
    #Save sensor footprint information
    file = open("./heterogeneous_EC/saved data/agents/sensor_footprint.txt", "a+")
    for agent in simulationData.pathPlanner.getAllAgents():
        file.write(str(agent.sensor_footprint[0]) + " ")
    file.write('-1 ')
    file.close()
    ...
    f = open("./heterogeneous_EC/saved data/paths/data2.txt", 'a+')
    ...
    #Save agent steps and info gained
